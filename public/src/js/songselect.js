class SongSelect {
	...
	init(fromTutorial, fadeIn, touchEnabled, songId, showWarning) {
		...
		if (this.siteMessagesEnabled) {
			...
			this.siteMessages = null
			var siteMessageButton = document.getElementById("site-message-button")
			var siteMessageOverlay = document.getElementById("site-message-overlay")
			if (siteMessageButton) {
				siteMessageButton.hidden = true
			}
			if (siteMessageOverlay) {
	...
	openSearchFromButton(event) {
		...
		if (this.state.screen === "song" && !this.search.opened && !(this.topSongs && this.topSongs.opened) && !this.weeklyChallenge.opened && !(typeof window !== "undefined" && window.EasySettings && window.EasySettings.isOpen && window.EasySettings.isOpen())) {
	...
	openTopSongsFromButton(event) {
		event.preventDefault()
		event.stopPropagation()
		if (this.topSongs && this.state.screen === "song" && !this.search.opened && !this.topSongs.opened && !this.weeklyChallenge.opened && !(typeof window !== "undefined" && window.EasySettings && window.EasySettings.isOpen && window.EasySettings.isOpen())) {
	...
	openEasySettingsFromButton(event) {
		event.preventDefault()
		event.stopPropagation()
		if (typeof window !== "undefined" && window.EasySettings && this.state.screen === "song" && !this.search.opened && !(this.topSongs && this.topSongs.opened) && !this.weeklyChallenge.opened && !this.uploadModal.opened) {
	...
	mouseMove(event) {
		...
		} else if (this.state.screen === "song" && !this.search.opened && !(this.topSongs && this.topSongs.opened)) {
			if (20 < mouse.y && mouse.y < 90 && 410 < mouse.x && mouse.x < 880 && (mouse.x < 540 || mouse.x > 750)) {
	...
	toSelectDifficulty(fromP2, playVoice = true) {
		...
		if (p2.session && !fromP2 && (!currentSong.action || !currentSong.p2Enabled)) {
			...
			if (currentSong.courses) {
				...
				this.search.display(true)
			} else if (currentSong.action === "topSongs" && this.topSongs) {
				this.topSongs.display(true)
