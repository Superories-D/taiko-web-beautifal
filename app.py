...
def route_api_songs():
    ...
    songs = [serialize_song(song) for song in db.songs.find(query)]
...
def current_weekly_challenge(now=None):
    ...
    candidates = list(db.songs.find(query).sort('id', 1))
    if not candidates:
        return None
...
def int_score_field(data, name):
    try:
        return max(0, int(float(data.get(name, 0))))
